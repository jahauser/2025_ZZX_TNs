...
for f in files
    @load f L T lambda delta q samples obs mean_data var_data
    key = (L, T, lambda, delta, q)

    if !haskey(results, key)
        # Deepcopy so we don't mutate original arrays from the first file
        results[key] = (deepcopy(mean_data), deepcopy(var_data), samples, obs)
    else
        acc_mean, acc_var, acc_samples, acc_obs = results[key]
        @assert acc_obs == obs "Observable mismatch for $f"
        for s in obs
            acc_mean[s] .+= mean_data[s] .* samples
            acc_var[s]  .+= var_data[s]  .* samples
        end
        results[key] = (acc_mean, acc_var, acc_samples + samples, acc_obs)
    end
end

# Normalize weighted sums to averages
for (key, (m, v, total_samples, obs)) in results
    for s in obs
        m[s] ./= total_samples
        v[s] ./= total_samples
    end
    results[key] = (m, v, total_samples, obs)
end
